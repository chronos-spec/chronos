...
// Instant temporel partagé avec la frise : permet d'atténuer les espèces qui
// n'existaient pas à ce moment, pour "zoomer" l'arbre sur une époque.
const FocusYaCtx = createContext(null);
// Une espèce est présente à l'instant T (années avant aujourd'hui) si elle est
// déjà apparue (from >= T) et pas encore éteinte (to <= T, ou vivante).
function aliveAt(node, T) {
  if (T == null || node.from == null) return true;
  const end = node.to == null ? 0 : node.to;
  return node.from >= T && end <= T;
}
...
function Node({node, depth=0, highlight=false}) {
  ...
  // Synchronisation avec la frise : les espèces absentes à cet instant s'estompent.
  const present = focusYa != null ? aliveAt(node, focusYa) : null;

  return (
    <div style={{marginLeft:depth*16,marginBottom:1,opacity:present===false?.32:1,transition:"opacity .35s ease"}}>
      <div
...
function NodeWithRef({node, depth=0, targetId=null, nodeRefs}) {
  ...
  // Présence à l'instant sélectionné sur la frise (pour estomper les absents).
  const present = focusYa != null ? aliveAt(node, focusYa) : null;
  ...
  return (
    <div ref={nodeRef} style={{marginLeft:depth*16,marginBottom:1,
      opacity: present===false ? .3 : 1, transition:"opacity .35s ease"}}>
      <div
        style={{display:"flex",alignItems:"center",gap:5,padding:"4px 8px",
          borderRadius:5,cursor:"pointer",transition:"background .1s",
          background: isTarget
...
export function LifeTree({ onFocusTimeline, onLocateSpecies, focusYa = null } = {}) {
  ...
  // vivant (≤ ~3,8 Ga) : inutile d'estomper tout l'arbre en vue cosmique.
  const activeFocus = (focusYa != null && focusYa <= 3.8e9) ? focusYa : null;
  // Contemporains : espèces présentes à cet instant.
  const contemporaries = useMemo(() => {
    if (activeFocus == null) return [];
    return ALL_NODES
      .filter(n => aliveAt(n, activeFocus))
