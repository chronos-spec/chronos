...
// Toutes les dates ci-dessous sont en "années avant aujourd'hui" (2025), comme
// partout ailleurs dans l'app. Certaines valeurs historiques (ex. Empire romain,
// Croisades, Alexandre le Grand) étaient saisies comme années de calendrier
// brutes (ex. "753" pour 753 av. J.-C.) au lieu d'années-avant-aujourd'hui,
// ce qui les plaçait au mauvais endroit sur la frise — corrigé ici.
const THEMES = {
  civilisations:{ label:"Civilisations & Empires", color:"#f59e0b", icon:"🏛️", items:[
    { id:"sumer",    label:"Sumer",                from:6525,  to:3925,  color:"#f59e0b" },
    { id:"egypte",   label:"Égypte ancienne",      from:5125,  to:2055,  color:"#eab308" },
    { id:"grece",    label:"Grèce antique",         from:2825,  to:2171,  color:"#3b82f6" },
    { id:"rome",     label:"Empire romain",         from:2778,  to:1549,  color:"#ef4444" },
    { id:"byzance",  label:"Empire byzantin",       from:1695,  to:572,   color:"#8b5cf6" },
    { id:"perse",    label:"Empire perse",          from:2575,  to:2355,  color:"#f97316" },
    { id:"islam",    label:"Âge d'or islamique",    from:1393,  to:767,   color:"#10b981" },
    { id:"ottoman",  label:"Empire ottoman",        from:726,   to:103,   color:"#059669" },
    { id:"chine_h",  label:"Chine impériale",       from:2246,  to:113,   color:"#dc2626" },
    { id:"mongol",   label:"Empire mongol",         from:819,   to:657,   color:"#78716c" },
    { id:"maya",     label:"Civilisation maya",     from:4025,  to:525,   color:"#16a34a" },
    { id:"aztec",    label:"Empire aztèque",        from:680,   to:504,   color:"#15803d" },
    { id:"inca",     label:"Empire inca",           from:587,   to:492,   color:"#166534" },
    { id:"brit",     label:"Empire britannique",    from:442,   to:28,    color:"#1d4ed8" },
    { id:"france",   label:"France royale",         from:1038,  to:233,   color:"#2563eb" },
  ]},
  religions:{ label:"Religions & Philosophies", color:"#8b5cf6", icon:"✨", items:[
    { id:"hindou",   label:"Hindouisme",            from:3500,  to:null,  color:"#f97316" },
    { id:"judaisme", label:"Judaïsme",              from:3300,  to:null,  color:"#3b82f6" },
  ...
  guerres:{ label:"Guerres & Conflits", color:"#ef4444", icon:"⚔️", items:[
    { id:"croisades",label:"Croisades",             from:929,   to:734,   color:"#7c3aed" },
    { id:"gua",      label:"Guerre de Cent Ans",    from:688,   to:572,   color:"#2563eb" },
  ...
  arts:{ label:"Arts & Culture", color:"#ec4899", icon:"🎨", items:[
    ...
    { id:"renaiss",  label:"Renaissance italienne", from:625,   to:425,   color:"#ec4899" },
    { id:"baroque",  label:"Baroque",               from:425,   to:275,   color:"#f97316" },
  ...
  personnages:{ label:"Personnages clés", color:"#f97316", icon:"👤", items:[
    { id:"alex",     label:"Alexandre le Grand",    from:2381,  to:2348,  color:"#6366f1" },
    { id:"cesar",    label:"Jules César",           from:2125,  to:2069,  color:"#ef4444" },
...
export function drawAll(canvas, miniCanvas, params) {
  ...
  if(miniCanvas){
    ...
    mgrd.addColorStop(0,"#efe7d6");mgrd.addColorStop(1,"#e6dcc6");
    ...
    for(const rect of CHRONO_RECTS.filter(r=>r.depth===1)){
      ...
      mctx.fillStyle=rect.color+"aa";mctx.fillRect(Math.max(0,ex1),2,Math.abs(ex2-ex1),mh-4);
    }
    for(const ev of ALL_EVENTS.filter(e=>e.importance===1)){
      const lv=L(ev.yearsAgo);if(lv<tle||lv>tls)continue;
      const mx=(tls-lv)/tR*mw;
      mctx.beginPath();mctx.arc(mx,mh/2,2,0,Math.PI*2);mctx.fillStyle=cc(ev.cat)+"ee";mctx.fill();
    }
