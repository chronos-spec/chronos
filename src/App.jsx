...
export default function Chronos() {
  ...
  const fetchZone=useCallback(async(startYa,endYa)=>{
    ...
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":import.meta.env.VITE_ANTHROPIC_KEY,"anthropic-version":"2023-06-01"},
...
[{"yearsAgo":number,"title":"titre court max 6 mots","date_label":"date précise lisible","desc":"1-2 phrases de description factuelle","cat":"cosmique|geologie|biologique|prehistoire|histoire","relevance":"lien avec la recherche en 4 mots max"}]
...
Si aucun événement réel ne correspond, retourne [].`}]})});
  ...
  useEffect(()=>{
    ...
    const onWheel=(e)=>{
      ...
      _zoom.current(makeCoord(S.current.vs,S.current.ve,cnv.width).toYa(e.clientX-rect.left),e.deltaY>0?1.13:.88);
      _redraw.current();
      _fetch.current();
    };
    ...
    const onMM=(e)=>{
      ...
      if(nid!==s.hoveredId){
        ...
        if(found){
          let tx=mx+16,ty=my-68;
          if(tx+220>cnv.width)tx=mx-226;
          if(ty<10)ty=my+20;
    ...
    // 5. Touch multitouch (mobile)
    let lt=null,ld=null;
    const onTS=(e)=>{
      if(e.touches.length===1)lt=e.touches[0].clientX;
      else if(e.touches.length===2)ld=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);
    };
    const onTM=(e)=>{
      e.preventDefault();
      const rect=cnv.getBoundingClientRect(),s=S.current;
      if(e.touches.length===1&&lt!==null){
        const dx=e.touches[0].clientX-lt;
        ...
        _zoom.current(makeCoord(s.vs,s.ve,cnv.width).toYa(mx),ld/d);
        ld=d;
        _redraw.current();
        _fetch.current();
      }
    ...
    return()=>{
      ...
      cnv.removeEventListener("touchstart",onTS);
      cnv.removeEventListener("touchmove",onTM);
      cnv.removeEventListener("touchend",onTE);
  ...
  return (
    <div style={css.app}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Mono:wght@300;400;500&display=swap');@keyframes dp{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.3;transform:scale(.5)}}@keyframes spin{to{transform:rotate(360deg)}}*{box-sizing:border-box}`}</style>
