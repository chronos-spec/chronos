  ...
  useEffect(()=>{
    (async()=>{
      ...
      try{
        ...
        if(bm && bm !== "null") setBookmarks(JSON.parse(bm));
      }catch(e){}
      try{
        // Load custom tags
        const tags=JSON.parse(localStorage.getItem("chronos-tags") || "null");
        if(tags && tags !== "null") setCustomTags(JSON.parse(tags));
      }catch(e){}
    })();
  },[]);
...
Si aucun événement réel ne correspond, retourne [].`}]})});
  ...
  const css={
    ...
    brandLine:{fontSize:11,color:"rgba(18,16,14,.45)",lineHeight:1.5,marginTop:4},
    ...
    sbSection:{padding:"14px 20px 12px",borderBottom:"1px solid rgba(18,16,14,.06)"},
    ...
    epochPill:(ep)=>({display:"flex",alignItems:"center",gap:6,padding:"6px 10px",borderRadius:8,border:`1px solid ${ep.stripe}33`,background:ep.pillBg,color:ep.pillText,fontSize:11,cursor:"pointer",transition:"all .15s",whiteSpace:"nowrap",overflow:"hidden",fontFamily:"'DM Mono',monospace"}),
    ...
    searchInput:{height:34,width:"100%",borderRadius:8,border:"1px solid rgba(18,16,14,.18)",background:"#fffaf3",padding:"0 32px 0 30px",fontSize:12,fontFamily:"'DM Mono',monospace",color:"#12100e",outline:"none",boxSizing:"border-box",transition:"border-color .2s"},
    ...
    searchDropdown:{position:"absolute",top:"calc(100% + 4px)",left:0,right:0,background:"#faf7f2",border:"1px solid rgba(18,16,14,.14)",borderRadius:8,boxShadow:"0 8px 28px rgba(18,16,14,.14)",zIndex:200,overflow:"hidden"},
    ...
    searchAiBadge:{padding:"2px 7px",borderRadius:10,background:"rgba(200,150,60,.1)",border:"1px solid rgba(200,150,60,.2)",fontSize:8,color:"#c8963c",letterSpacing:".08em"},
    searchItem:{display:"flex",alignItems:"flex-start",gap:9,padding:"9px 12px",cursor:"pointer",borderBottom:"1px solid rgba(18,16,14,.05)",transition:"background .12s",width:"100%",background:"transparent",border:0,textAlign:"left",fontFamily:"'DM Mono',monospace"},
    ...
    searchItemTitle:{fontSize:13,fontWeight:500,color:"#12100e",fontFamily:"Georgia,serif"},
    searchItemDate:{fontSize:10,color:"#b17a25"},
    ...
    searchItemRel:{fontSize:9,color:"rgba(18,16,14,.35)",fontStyle:"italic"},
    ...
    sbAction:{flex:1,padding:"9px 0",borderRadius:8,background:"#f0ebe0",color:"#12100e",border:"1px solid rgba(18,16,14,.12)",fontFamily:"'DM Mono',monospace",fontSize:11,cursor:"pointer",transition:"all .15s"},
    // MAIN
    main:{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0},
    mainHeader:{padding:"22px 28px 14px",borderBottom:"1px solid rgba(18,16,14,.08)",background:"#faf7f2",flexShrink:0,display:"flex",alignItems:"flex-start",justifyContent:"space-between"},
    eyebrow:{fontSize:10,letterSpacing:".16em",textTransform:"uppercase",color:"#c8963c",marginBottom:5},
    pageTitle:{fontFamily:"Georgia,serif",fontSize:38,fontWeight:400,color:"#12100e",lineHeight:1.1,margin:"0 0 6px"},
    pageSubtitle:{fontSize:12,color:"rgba(18,16,14,.5)"},
    ...
    // CARDS
    cardsRow:{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:10,padding:"14px 28px",flexShrink:0},
    card:(c)=>({display:"flex",flexDirection:"column",padding:"14px 14px",borderRadius:12,background:c.bg,border:`1px solid ${c.color}22`,cursor:"pointer",transition:"all .18s",fontFamily:"'DM Mono',monospace",textAlign:"left"}),
    cardIcon:(c)=>({width:36,height:36,borderRadius:9,background:c,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontFamily:"Georgia,serif",fontSize:17,fontWeight:700,marginBottom:10,flexShrink:0}),
    ...
    // TIMELINE
    timelineCard:{flex:1,margin:"0 28px 14px",background:"#faf7f2",borderRadius:12,border:"1px solid rgba(18,16,14,.10)",overflow:"hidden",display:"flex",flexDirection:"column",minHeight:0},
    timelineBar:{padding:"8px 14px",background:"#faf7f2",borderBottom:"1px solid rgba(18,16,14,.07)",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0},
    ...
    zoomBtns:{position:"absolute",right:10,top:12,display:"flex",gap:5,zIndex:20},
    ...
    bmBtn:(active,col)=>({display:"inline-flex",alignItems:"center",gap:4,padding:"4px 10px",borderRadius:12,fontSize:10,letterSpacing:".08em",fontWeight:500,cursor:"pointer",border:`1.5px solid ${col||"rgba(18,16,14,.2)"}`,background:active?`${col||"#c8963c"}18`:"transparent",color:active?(col||"#c8963c"):"rgba(18,16,14,.5)",transition:"all .15s",whiteSpace:"nowrap",fontFamily:"'DM Mono',monospace"}),
    ...
    bmTag:(col)=>({display:"inline-flex",alignItems:"center",padding:"2px 7px",borderRadius:8,fontSize:9,border:`1px solid ${col}44`,background:`${col}12`,color:col,flexShrink:0,marginTop:1}),
  };
  ...
  return (
    <div style={css.app}>
      <style>{`
        ...
        .ep-pill:hover{opacity:.85;transform:translateY(-1px)!important}
        .per-pill:hover{transform:translateY(-1px)!important}
        .card-btn:hover{transform:translateY(-2px);box-shadow:0 4px 16px rgba(18,16,14,.1)!important}
        button,input{font-family:'DM Mono',monospace}
      `}</style>
      ...
      <aside style={css.sidebar}>
        ...
        <div style={css.sbSection}>
          ...
          <div style={css.searchWrap}>
            ...
            <input style={css.searchInput} placeholder="Rechercher : Lune, Rome, dinosaures..."
            ...
            {ui.searchOpen&&ui.searchQuery&&(
              <div style={css.searchDropdown}>
                ...
                {ui.searchResults.map((ev,i)=>(
                  <button key={ev.id||i} className="srch-item" style={css.searchItem} onMouseDown={e=>{e.preventDefault();goToResult(ev);}}>
        ...
        <div style={css.sbSection}>
          ...
          <div style={css.epochGrid}>
            {EPOCHS.map(ep=>(
              <button key={ep.label} className="ep-pill" style={css.epochPill(ep)} onClick={()=>navigateToEpoch(ep)}>
                <span style={css.epochDot(ep.stripe)}/>
                <span style={{overflow:"hidden",textOverflow:"ellipsis"}}>{ep.label}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div style={css.sbSection}>
          ...
          <div style={css.periodWrap}>
            {PERIODS.map(per=>(
              <button key={per.label} className="per-pill" style={css.periodPill(per)} onClick={()=>navigateToEpoch({from:per.from,to:per.to})}>
                {per.label}
              </button>
            ))}
          </div>
      ...
      <div style={css.main}>
        ...
        <div style={css.cardsRow}>
          {EXPLORE_CARDS.map(c=>(
            <button key={c.title} className="card-btn" style={css.card(c)} onClick={()=>navigateToEpoch({from:c.from,to:c.to})}>
            ...
            <div style={css.legend(ui.legendOpen)}>
              ...
              {[["●","Majeur — toujours visible"],["◦","Notable — zoom ×2+"],["·","Contextuel — zoom ×5+"]].map(([s,l])=>(
                <div key={l} style={{...css.legItem,marginBottom:5,alignItems:"flex-start"}}>
                  <span style={{fontSize:14,color:"#8a6000",flexShrink:0,marginTop:1}}>{s}</span>
              ...
              <div style={{...css.legHead,marginTop:13}}>Arbre de la vie</div>
              {LIFE_TREE.slice(0,8).map(s=>(
                <div key={s.id} style={css.legItem}><div style={css.legBar(s.color)}/><span style={css.legLbl}>{s.label}</span></div>
              ))}
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                    {customTags.map((tag,i)=>(
                      <div key={tag} style={{display:"flex",alignItems:"center",gap:4,padding:"3px 9px",borderRadius:10,background:"rgba(18,16,14,.05)",border:"1px solid rgba(18,16,14,.1)",fontSize:11}}>
                        ...
                        <button style={{width:14,height:14,borderRadius:"50%",background:"none",border:"none",cursor:"pointer",fontSize:9,color:"rgba(18,16,14,.4)"}} onClick={()=>removeCustomTag(tag)}>✕</button>
                      </div>
                    ))}
                    {addingTag?(
                      <div style={{display:"flex",gap:4}}>
                        <input autoFocus value={newTagInput} onChange={e=>setNewTagInput(e.target.value)}
                          ...
                          style={{width:90,height:24,border:"1px solid rgba(18,16,14,.2)",borderRadius:5,padding:"0 7px",fontSize:11,outline:"none"}} placeholder="Nom..."/>
                        <button style={{width:24,height:24,borderRadius:"50%",background:"#0a7848",color:"#fff",border:"none",cursor:"pointer",fontSize:11}} onClick={()=>addCustomTag(newTagInput)}>✓</button>
                      </div>
                    ):(
                      <button style={{padding:"3px 9px",borderRadius:10,border:"1px dashed rgba(18,16,14,.22)",fontSize:11,color:"rgba(18,16,14,.5)",cursor:"pointer",background:"transparent"}} onClick={()=>setAddingTag(true)}>+ Ajouter</button>
                    )}
                  </div>
                </div>
                <div style={{flex:1,overflowY:"auto"}}>
                  {Object.keys(bookmarks).length===0&&(
                    <div style={{padding:"28px 16px",textAlign:"center",fontSize:12,color:"rgba(18,16,14,.42)",lineHeight:1.6}}>
                      Aucun signet.<br/><span style={{fontSize:11}}>Ouvrez un événement et cliquez 🔖</span>
                    </div>
                  )}
                  {Object.entries(bookmarks).map(([evId,bm])=>{
                    ...
                    return(
                      <div key={evId} className="srch-item" style={css.bmViewItem} onClick={()=>goToResult(ev)}>
                        ...
                        <span style={{fontSize:13,color:"rgba(18,16,14,.3)",cursor:"pointer",flexShrink:0}} onClick={e=>{e.stopPropagation();removeBookmark(evId);}}>✕</span>
                      </div>
                    );
                  })}
                </div>
