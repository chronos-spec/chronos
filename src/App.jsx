...
export default function Chronos() {
  ...
  useEffect(()=>{
    (async()=>{
      try{const cached=JSON.parse(localStorage.getItem("chronos-cache")||"null");if(cached&&typeof cached==="object")Object.assign(S.current.panelCache,cached);}catch(e){}
  ...
  const saveCache=useCallback(async()=>{try{localStorage.setItem("chronos-cache",JSON.stringify(S.current.panelCache));}catch(e){}},[]);
  const saveBookmarks=useCallback(async(bm)=>{try{localStorage.setItem("chronos-bookmarks",JSON.stringify(bm));}catch(e){}},[]);
  const saveTags=useCallback(async(tags)=>{try{localStorage.setItem("chronos-tags",JSON.stringify(tags));}catch(e){}},[]);
...
En HTML simple (<p>,<h3>,<strong> uniquement). Sections : intro immersive (1§), <h3>Contexte</h3>(1§), <h3>Ce qui s'est passé</h3>(1§), <h3>Héritage</h3>(1§), <h3>Le saviez-vous ?</h3>(1§). ~260 mots. HTML direct, sans balises html/body.`}]})});
      ...
      saveCache();
    }catch(e){setUi(u=>({...u,panelContent:"<p>La fiche détaillée n'a pas pu être chargée.</p>",panelError:"Connexion à l'IA indisponible. Réessayez en rouvrant cette fiche.",panelEventId:ev.id}));}
  },[saveCache]);
  ...
  const openPeriodPanel=useCallback((item)=>{
    ...
    setUi(u=>({...u,panelOpen:true,panelCat:item.from>1e9?"ÈRE":"PÉRIODE",panelCatColor:item.stripe||item.color||"#c8963c",panelDate:fmt(item.from)+" → "+(item.to>0?fmt(item.to):"aujourd'hui"),panelTitle:item.label,panelContent:html,panelError:null,tooltip:null,panelEventId:"period_"+item.label,showBookmarkMenu:false}));
  ...
  const searchWithAI=useCallback(async(query)=>{
    ...
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":import.meta.env.VITE_ANTHROPIC_KEY,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},
        body:JSON.stringify({model:"claude-3-5-sonnet-20241022",max_tokens:1000,messages:[{role:"user",content:`Tu es un historien expert. L'utilisateur recherche : "${query}"\n\nTrouve les événements les plus pertinents. Réponds UNIQUEMENT par un tableau JSON :\n[{"yearsAgo":number,"title":"titre court max 6 mots","date_label":"date lisible","desc":"1-2 phrases","cat":"cosmique|geologique|biologique|prehistoire|histoire","relevance":"4 mots max"}]\nSi aucun événement, retourne [].`}]})});
      if(!res.ok)throw new Error(res.status);
      const data=await res.json();
      const raw=(data.content||[]).find(b=>b.type==="text")?.text||"[]";
      const match=raw.match(/\[[\s\S]*?\]/);if(!match)throw new Error("no array");
    ...
    }catch(e){setUi(u=>({...u,searchLoading:false,searchDone:true,searchError:"Recherche IA indisponible. Résultats locaux affichés."}));}
  },[]);
  ...
  // ── REFS STABLES : le useEffect ne se réenregistre qu'une seule fois ──────
  ...
  useEffect(()=>{
    ...
    const onWheel=(e)=>{e.preventDefault();const rect=cnv.getBoundingClientRect();zoomAroundRef.current(makeCoord(S.current.vs,S.current.ve,cnv.width).toYa(e.clientX-rect.left),e.deltaY>0?1.13:.88);scheduleRedrawRef.current();triggerFetchRef.current();};
    let dragging=false;
    ...
    const onMM=(e)=>{
      ...
      if(dragging){const s=S.current,lr=L(s.vs)-L(s.ve),sh=-(e.movementX/cnv.width)*lr,ls=L(s.vs)+sh,le=L(s.ve)+sh;if(ls>Math.log10(UA*1.1)||le<0)return;s.vs=Math.pow(10,ls);s.ve=Math.pow(10,le);scheduleRedrawRef.current();triggerFetchRef.current();return;}
      ...
      if(nid!==s.hoveredId){s.hoveredId=nid;wrap.style.cursor=found?"pointer":"grab";scheduleRedrawRef.current();
    ...
    const onClick=(e)=>{
      ...
      if(s.periodY!=null&&my>=s.periodY&&my<=s.periodY+(s.periodH||20)){const ya=makeCoord(s.vs,s.ve,cnv.width).toYa(mx);const per=PERIODS.find(p=>ya<=p.from&&ya>=p.to);if(per){openPeriodPanelRef.current(per);return;}}
      if(s.periodY!=null&&my<s.periodY&&my>44){const ya=makeCoord(s.vs,s.ve,cnv.width).toYa(mx);const ep=EPOCHS.find(p=>ya<=p.from&&ya>=p.to);if(ep){openPeriodPanelRef.current(ep);return;}}
  ...
  },[]);// ← dépendances vides : s'enregistre UNE SEULE FOIS
        ...
        <main className="chronos-main" style={css.main}>
          ...
          <section style={css.timelineCard}>
            ...
            <div ref={wrapRef} style={css.wrap}>
              ...
              <EventPanel ui={ui} bookmarks={bookmarks} customTags={customTags} addingTag={addingTag} newTagInput={newTagInput} setAddingTag={setAddingTag} setNewTagInput={setNewTagInput} addCustomTag={addCustomTag} toggleBookmark={toggleBookmark} closePanel={closePanel} stateRef={S}/>
            </div>
          </section>
