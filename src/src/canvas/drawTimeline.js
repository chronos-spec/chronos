...
export function drawAll(canvas, miniCanvas, params) {
  ...
  // Bas de la zone de dessin (l'arbre de vie du canvas a été retiré)
  const BOTTOM  = H - 6;
  const EVT_TOP = LINE_Y;
  const EVT_BOT = BOTTOM - 14;
  ...
  for(const ep of EPOCHS){
    ...
    if(x1>1&&x1<W-1){
      ...
      ctx.beginPath();ctx.moveTo(x1,3);ctx.lineTo(x1,BOTTOM);ctx.stroke();
    }
    // Label ère
    if(rw>50){
  ...
  for(let ya=Math.ceil(ve/chosen)*chosen;ya<=vs;ya+=chosen){
    ...
    ctx.beginPath();ctx.moveTo(x,LINE_Y);ctx.lineTo(x,BOTTOM);ctx.stroke();
  }
  ...
  if(nowX>2&&nowX<W-2){
    ...
    ctx.beginPath();ctx.moveTo(nowX,LINE_Y);ctx.lineTo(nowX,BOTTOM);ctx.stroke();
    ctx.setLineDash([]);
  ...
  return {placed, LINE_Y, PERIOD_Y, PERIOD_H, TREE_TOP:BOTTOM, bandRects:[]};
}
